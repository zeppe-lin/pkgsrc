...
======================================================================
= AFTER EDITING THIS HANDBOOK, DO NOT FORGET TO UPDATE THE WEBSITE,  =
= SINCE IT IS GENERATED FROM THIS HANDBOOK:                          =
=                                                                    =
=   $ git clone git@github.com:zeppe-lin/zeppe-lin.github.io         =
=   $ cd zeppe-lin.github.io                                         =
=   $ make && make update                                            =
======================================================================
...
<img src="/images/logo.png" alt="Zeppe-Lin Logo" class="logo">

=end html

This handbook (as a whole Zeppe-Lin) is inspired and based on the
L<CRUX Handbook|https://crux.nu/Main/Documentation>.

=begin html

It's also available on the Zeppe-Lin system as B<handbook(7)>
manual page.
...
Since Zeppe-Lin is a GNU/Linux distribution, it contains software
...
All package build scripts in Zeppe-Lin are Copyright (c) 2000-2021
...
Using L<parted(8)|https://www.mankier.com/8/parted> utility we can
create all required partitions.
...
This is a quiet simple logical volumes scheme used in this handbook.
...
For example, we have 4GB RAM. The swap will have 8GB.
...
=head3 Download rootfs tarball
  ...
  # cd /mnt
  # wget -c https://github.com/zeppe-lin/zeppe-lin/releases/download/2022-12-08/rootfs-2022-12-08-x86_64.tar.xz{,.sig}

=head4 Signature verification

  # gpg --keyserver keyserver.ubuntu.com --recv-keys 59ec1986fbd902cf
  # gpg --verify rootfs-2022-12-08-x86_64.tar.xz.sig \
    rootfs-2022-12-08-x86_64.tar.xz

=head3 Extract rootfs tarball

  # tar -xvf rootfs-2022-12-08-x86_64.tar.xz -C .
...
multiple disks.  Use
L<blkid(8)|https://www.mankier.com/8/blkid> to verify the disk's UUID.
...
and services.  See
L<rc.conf(5)|https://zeppe-lin.github.io/rc.conf.5.html> for more info.
...
bridge interface to dhcpcd and/or wpa_supplicant rc files.
...
L<mkinitramfs(8)|https://zeppe-lin.github.io/mkinitramfs.8.html> and
updates the grub config.  We turn off the execution of this script
because we'll do it manually.  It is useful when regularly updating
the system.
...
Since the L<pkgman(1)|https://zeppe-lin.github.io/pkgman.1.html> build
the package that contains only the binaries (kernel and modules), you
may want to build your own kernel.  For example, you do not need such
a fat kernel, which goes by default, or you need the sources of the
running kernel because you want to build the nvidia driver and/or
virtualbox.
...
Add the mkinitramfs configuration file (F</etc/mkinitramfs/config>)
...
machine.
Use L<blkid(8)|https://www.mankier.com/8/blkid> to obtain C<UUID>.
...
L<pkgadd(8)|https://zeppe-lin.github.io/pkgadd.8.html>.  This helps
distinguish packages from other I<tar.gz> files.  Note that
L<pkgmk(8)|https://zeppe-lin.github.io/pkgmk.8.html> supports
additional compression schemes like B<bzip2> with the I<tar.bz2>
extension or B<XZ> ending with I<tar.xz>.

L<pkgadd(8)|https://zeppe-lin.github.io/pkgadd.8.html>,
L<pkgrm(8)|https://zeppe-lin.github.io/pkgrm.8.html>, and
L<pkginfo(1)|https://zeppe-lin.github.io/pkginfo.1.html> are the basic
package management utilities and are part of the package B<pkgutils>.
In addition to them, package management includes utilities such as
L<pkgmk(8)|https://zeppe-lin.github.io/pkgmk.8.html>,
L<rejmerge(8)|https://zeppe-lin.github.io/rejmerge.8.html>, and
L<revdep(1)|https://zeppe-lin.github.io/revdep.1.html>.
...
L<pkgadd(8)|https://zeppe-lin.github.io/pkgadd.8.html>, a new record
is added to the package database (stored in F</var/lib/pkg/db>).  The
...
packages to be installed.  The included
L<pkgman(1)|https://zeppe-lin.github.io/pkgman.1.html> tool
(B<pkgman>), however, does support dependencies.
...
be found on their respective manpage.
...
L<pkgadd(8)|https://zeppe-lin.github.io/pkgadd.8.html>.  This utility
...
L<pkgadd(8)|https://zeppe-lin.github.io/pkgadd.8.html> will abort
without installing the package.  The error message will contain the
names of the conflicting files.  Example:
...
can use the option B<-f> (or B<--force>).
Example:
...
installed files it could be a sign that the package is broken and
installs unexpected files.  Use this option with extreme care,
...
Instead, the L<pkgadd(8)|https://zeppe-lin.github.io/pkgadd.8.html>
...
L<pkgadd(8)|https://zeppe-lin.github.io/pkgadd.8.html> will interpret
this as a package named I<bash> at version I<5.0.18-1>.  If
L<pkgadd(8)|https://zeppe-lin.github.io/pkgadd.8.html> is unable to
interpret the filename (e.g. B<#> is missing or the filename does not
end with .pkg.tar.gz) an error message will be printed and
L<pkgadd(8)|https://zeppe-lin.github.io/pkgadd.8.html> will abort
...
L<pkgadd(8)|https://zeppe-lin.github.io/pkgadd.8.html> with the B<-u>
option.  Example:
...
L<pkgadd(8)|https://zeppe-lin.github.io/pkgadd.8.html> will
print an error message.
L<pkgadd(8)|https://zeppe-lin.github.io/pkgadd.8.html> does not care
about the version number of the package in that you can "upgrade"
version 2.05-1 with version 2.04-1 (or even with version 2.05-1 itself).
The installed package will be replaced with the specified package.

Upgrading a package is equivalent to executing
L<pkgrm(8)|https://zeppe-lin.github.io/pkgrm.8.html> followed by
L<pkgadd(8)|https://zeppe-lin.github.io/pkgadd.8.html> with one (big)
exception.  When upgrading a package (with C<pkgadd -u>) you have the
option to prevent some of the already installed files from getting
replaced.  This is typically useful when you want to preserve
configuration and log files.

When executing
L<pkgadd(8)|https://zeppe-lin.github.io/pkgadd.8.html> the file
F</etc/pkgadd.conf> will be read.
This file can contain rules describing how
L<pkgadd(8)|https://zeppe-lin.github.io/pkgadd.8.html> should behave
when doing upgrades.  A rule is built out of three fragments:
I<event>, I<pattern> and I<action>.  The I<event> describes in what
kind of situation this rule applies.  Currently only one type of event
is supported, that is UPGRADE.  The I<pattern> is a filename pattern
expressed as a regular expression and the I<action> applicable to the
UPGRADE event is YES or NO.  More than one rule of the same event type
is allowed, in which case the first rule will have the lowest priority
...
except files in F</etc/X11/> (subdirectories included), unless it's
the file F</etc/X11/xorg.conf>.  The default rule is to upgrade
...
F</var/lib/pkg/rejected/>.  Files in this directory are never added to
the package database.  The user is then free to examine, use and/or
remove that file manually.  Another option is to use
L<rejmerge(8)|https://zeppe-lin.github.io/rejmerge.8.html>.  For each
rejected file found in F</var/lib/pkg/rejected/>,
L<rejmerge(8)|https://zeppe-lin.github.io/rejmerge.8.html> will
display the difference between the installed version and the rejected
version.  The user can then choose to keep the installed version,
upgrade to the rejected version or perform a merge of the two.
Example (using the above F</etc/pkgadd.conf>):
...
L<pkgrm(8)|https://zeppe-lin.github.io/pkgrm.8.html>.  This utility
...
L<pkginfo(1)|https://zeppe-lin.github.io/pkginfo.1.html>.  This
...
L<pkgmk(8)|https://zeppe-lin.github.io/pkgmk.8.html> for creating and
...
L<pkgman(1)|https://zeppe-lin.github.io/pkgman.1.html> exists.

L<pkgman(1)|https://zeppe-lin.github.io/pkgman.1.html> supports
dependency handling (with the caveat mentioned below) as well as some
overlap with B<pkgutils> features.
...
L<pkgman(1)|https://zeppe-lin.github.io/pkgman.1.html>
...
skip it, and go further.  In the end, it will show the
C<build-failed packages> in the report.  This behavior can have
...
We can tell L<pkgman(1)|https://zeppe-lin.github.io/pkgman.1.html> not
to skip the C<fails>, but to stop.  Then we can fix the package build
and start installation further.
...
L<pkgman(1)|https://zeppe-lin.github.io/pkgman.1.html> to skip
installation of already installed package(s).  It just ignores the
package and installing next in the listed order.  At the end of
installation procedure, all skipped packages will be in the report.

=head4 Viewing and updating outdated packages

Since the packages sources for Zeppe-Lin is distributed via
...
L<pkgman(1)|https://zeppe-lin.github.io/pkgman.1.html> should check in
its operation.  By default, the I<core>, I<system>, I<xorg>,
I<desktop>, and I<stuff> collections are enabled.

=item B<runscripts>

This option configures
L<pkgman(1)|https://zeppe-lin.github.io/pkgman.1.html> to run
C<pre-install>, C<post-install>, C<pre-remove>, and C<post-remove>
scripts if they exist in the package source directory.  These scripts
are run during F<install>, F<update>, F<sysup>, and F<remove>
operations.
...
L<pkgman(1)|https://zeppe-lin.github.io/pkgman.1.html> and
L<pkgman.conf(5)|https://zeppe-lin.github.io/pkgman.conf.5.html>.

=head2 Creating Packages

Creating a package is done using
L<pkgmk(8)|https://zeppe-lin.github.io/pkgmk.8.html>.  This utility
uses a file called Pkgfile (see
L<Pkgfile(5)|https://zeppe-lin.github.io/Pkgfile.5.html> for more info),
which contains information about the package (such as name, version,
etc) and the commands that should be executed in order to compile the
package in question.  To be more specific, the Pkgfile file is
actually a POSIX L<sh(1)|https://www.mankier.com/1p/sh> script, which
defines a number of variables (name, version, release and source) and
a function (build).  Below is an example of what a Pkgfile file might
look like.  The example shows how to package the
L<grep(1)|https://www.mankier.com/1/grep> utility.  Some comments are
...
In reality, you do not include all those comments, so the real
Pkgfile for L<grep(1)|https://www.mankier.com/1/grep> looks like this:
...
L<pkgmk(8)|https://zeppe-lin.github.io/pkgmk.8.html> will check the
...
that were installed in $PKG (if the F<.footprint> did not already
...
update the contents of the F<.footprint> file you simply do
...
using L<pkgadd(8)|https://zeppe-lin.github.io/pkgadd.8.html> and try
it out.  It's highly recommended to look at the Pkgfile in another
package(s), since looking at examples is a great way to learn.

See L<Pkgfile(5)|https://zeppe-lin.github.io/Pkgfile.5.html> for
additional information.

=head2 Adjusting/Configuring the Package Build Process

Many settings of to the package build process can be adjusted
by editing the L<pkgmk(8)|https://zeppe-lin.github.io/pkgmk.8.html>
...
L<pkgmk(8)|https://zeppe-lin.github.io/pkgmk.8.html> will store (if
downloading) and use source archives when building.

=item PKGMK_PACKAGE_DIR

This setting defines where
L<pkgmk(8)|https://zeppe-lin.github.io/pkgmk.8.html> will create
package files once the build process is complete.

=item PKGMK_WORK_DIR

This setting defines a work area that
L<pkgmk(8)|https://zeppe-lin.github.io/pkgmk.8.html> will use to build
the package.
...
L<pkgmk(8)|https://zeppe-lin.github.io/pkgmk.8.html> to attempt to
fetch all source archives from
I<http://fileserver.intranet/zeppe-lin/sources/> before falling back
to the source URL specified in the Pkgfile.  Multiple URLs can be
separated by spaces (the spaces in URL replace by C<%20>).

  PKGMK_SOURCE_DIR="/var/cache/pkgmk/sources"

This setting instructs
L<pkgmk(8)|https://zeppe-lin.github.io/pkgmk.8.html> to store and find
source archives in C</var/cache/pkgmk/sources>.  An example benefit of
this setup would be the ability to store C</var/cache/pkgmk/sources>
on an NFS server on your local network for use by multiple Zeppe-Lin
installations.  C<PKGMK_PACKAGE_DIR> can be set and used the same way.

  PKGMK_WORK_DIR="/var/cache/pkgmk/work/$name"

This setting instructs
L<pkgmk(8)|https://zeppe-lin.github.io/pkgmk.8.html> to use
...
be to use a L<tmpfs(5)|https://www.mankier.com/5/tmpfs> as your work
directory.

There are a few more settings which can be found in the
L<pkgmk.conf(5)|https://zeppe-lin.github.io/pkgmk.conf.5.html> manual
page. 

=head2 Package Guidelines

A detailed guideline is described in
L<Pkgfile(5)|https://zeppe-lin.github.io/Pkgfile.5.html> manual page.
...
L<pkgmk(8)|https://zeppe-lin.github.io/pkgmk.8.html>.  This means that
this directory at least has the files F<Pkgfile> (which is the package
build description) and F<.footprint> (which is used for regression
testing and contains a list of files this package is expected to
contain once it is built).  Further, a package source directory can
contain patches and/or other files needed for building the package,
even software sources.  It is important to understand that the actual
source code for the package is not necessarily present in package
source directory.  Instead the F<Pkgfile> contains an URL which points
to a location where the source can be downloaded.
...
=head3 What is the pkgsrc repository?

The term I<pkgsrc repository> refers to a remote repository
containing packages' sources.

=head3 pkgsrc collections

Packages' sources are organized in so called collections.  There are
five different layer of packages.

=head4 core

This collection contains packages required to create a basic system,
on the basis of which you can build your installation.
The official rootfs images are built based on this collection.

Of course, this collection shouldn't have dependencies outside itself.
To enforce this rule during maintenance see
L<Makefile|https://github.com/zeppe-lin/zeppe-lin/blob/master/Makefile>
in zeppe-lin repository,
L<finddeps-linked(1)|https://zeppe-lin.github.io/finddeps-linked.1.html>,
and L<revdep(1)|https://zeppe-lin.github.io/revdep.1.html>.

=head4 system

This collection contains packages required for minimal installation
on real hardware.  It contains loaders, filesystems, servers, and so
on.  Packages in this collection depend B<ONLY> on the core.

=head4 xorg

This collection contains packages required for X Window System.
These packages depend on the system and core.
...
These packages depend on the system, core, and xorg.

=head4 stuff

This collection contains packages that do not fit into the above
categories.  Packages in this collection may depend on any of the
collections listed above.

=head2 Using The Pkgsrc

=head3 Synchronizing your local pkgsrc repository

When Zeppe-Lin is installed for the first time the local pkgsrc
structure (F</usr/src/zeppe-lin/*>) is empty.  To obtains the sources
at first time, use C<git clone>.  Example:

  # cd /usr/src/
  # git clone https://github.com/zeppe-lin/zeppe-lin

To bring your local zeppe-lin up to date use C<git pull>.  Example:

  # cd /usr/src/zeppe-lin
  # git pull

=head3 Listing local zeppe-lin repository

When the local zeppe-lin repository has been updated the directory
C</usr/src/zeppe-lin/> will contain some package categories, C<core>,
C<system>, C<xorg>, C<desktop> and C<stuff>.
...
You can also use L<pkgman(1)|https://zeppe-lin.github.io/pkgman.1.html>
...
Add the B<--path> option to see the package source location:
  ...
  [i] /usr/src/zeppe-lin/system/alsa-lib
  [ ] /usr/src/zeppe-lin/system/alsa-ucm-conf
  [i] /usr/src/zeppe-lin/system/alsa-utils
...
To find out if the zeppe-lin repository carries packages that
are different (likely newer) compared to the versions currently
installed you can use
L<pkgman-diff(1)|https://zeppe-lin.github.io/pkgman-diff.1.html>.  If
versions differences are found, the output could look like this:
...
L<pkgman-install(8)|https://zeppe-lin.github.io/pkgman-install.8.html>.
...
L<pkgmk(8)|https://zeppe-lin.github.io/pkgmk.8.html> to build it.
Example:
...
L<pkgmk(8)|https://zeppe-lin.github.io/pkgmk.8.html> to download the
source(s) specified in F<Pkgfile> (in case the source are already
downloaded this option is ignored).  When the download is completed
...
L<pkgmk(8)|https://zeppe-lin.github.io/pkgmk.8.html> in an environment
faking root privileges.  Example:

  $ fakeroot pkgmk -d

The L<pkgman(1)|https://zeppe-lin.github.io/pkgman.1.html> already
uses it by default in Zeppe-Lin installation.

If the package was built successfully you can use
L<pkgadd(8)|https://zeppe-lin.github.io/pkgadd.8.html> to install or
upgrade it.  Example:
...
Fourth, set up the C<makecommand> in F<pkgman.conf>:
...
L<pkgman(1)|https://zeppe-lin.github.io/pkgman.1.html> (and its child
processes) on other.  Now, if you could renice
L<pkgman(1)|https://zeppe-lin.github.io/pkgman.1.html>
...
See L<inittab(5)|https://manpages.debian.org/unstable/sysvinit-core/inittab.5.en.html>
...
L<rc.conf(5)|https://zeppe-lin.github.io/rc.conf.5.html> for more
information about F<rc.conf> configuration file.
...
install C<dhcpcd> package and run F</etc/rc.d/dhcpcd> service.

=head4 Wireless network

Before using wireless networking, use
L<rfkill(8)|https://www.mankier.com/8/rfkill> to check whether
...
L<crypt(3)|https://www.mankier.com/3/crypt> function to authenticate
users you should make sure that these programs are linked against the
B<libcrypt> library (i.e. use C<-lcrypt> when linking) which contains
the C<SHA512> version of the crypt function (this version is backwards
compatible and understands C<DES> passwords as well).

=head1 REPORTING BUGS

The source code for this site is available at:
L<https://github.com/zeppe-lin/zeppe-lin.github.io>.
Please note that it is generated
(by L<pod2html(1)|https://www.mankier.com/1/pod2html>)
from the handbook, that is available at
L<https://github.com/zeppe-lin/zeppe-lin>.
If you'd like to fix some C<CSS> bugs and build files, refer to the
source code of the site, otherwise to the handbook source.

Also, note that almost everything in Zeppe-Lin is distributed as a
package, so if you found any bug in the documentation and code, refer
to the Zeppe-Lin source code: L<https://github.com/zeppe-lin>.

=cut

# vim:ft=pod:sw=2:ts=2:sts=2:et:cc=72:tw=70
# End of file.
