...
B<pkgman> install [B<-v>|B<-vv>] [B<--test>] [B<--log>]
...
Install all packages in the listed order.  It looks in all
...
Install all packages in the listed order including their
dependencies.
...
Install all packages in the listed order, but stop if
installation of one package fails.  This can be used to
install packages where I<package2> depends on I<package1>.
