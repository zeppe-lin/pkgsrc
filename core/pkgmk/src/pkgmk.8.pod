...
for more information how-to write Pkgfile files.  Once a suitable
F<Pkgfile> file exists, each time you change some source files, you
simple execute B<pkgmk> to bring the package up to date.  The B<pkgmk>
program uses F<Pkgfile> file and the last-modification times of the
source files to decide if the package needs to be updated.

Global configuration file is stored in F</etc/pkgmk.conf>.
This file is read by B<pkgmk> at startup.
...
Use alternative configuration file (default is
F</etc/pkgmk.conf>).
...
L<Pkgfile(5)|https://zeppe-lin.github.io/Pkgfile.5.html> for more
information.
...
L<pkgmk.conf(5)|https://zeppe-lin.github.io/pkgmk.conf.5.html> for
more information.
